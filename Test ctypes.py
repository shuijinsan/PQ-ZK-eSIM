...
"""
test_ctypes.py — v5.0
Python ctypes binding verification

v5.2 changes from v4.0:
  - PQC_eUICC_Init: added salt, cred_kyc params
  - PQC_MerkleTree_Build/VerifyPath: added salt
  - New KAT: Merkle tree build  path verify (with salt)
  - New error codes: NOT_INITIALIZED, YSEC_CONSUMED
  - KAT2: R_dynamic expected from C test_vectors
  - Removed duplicate import os
  - New failure tests: tampered AuthToken, replay

Usage:
    python3 test_ctypes.py --so ./build/libpqzkesim_verify.so

Note:
    KAT2 c_layer expected values from C test_vectors,
    copy R_dynamic hex from output, currently placeholder.
"""
...
# ── Constants (aligned with pq_zk_esim.h)────────────────────────
PQ_ZK_N                = 256
PQ_ZK_K                = 3
PQ_ZK_SEED_BYTES       = 32
PQ_ZK_HASH_BYTES       = 32
...
# Merkle tree params (aligned with pqzk_merkle.h)
...
PQ_ZK_ERR_NOT_INITIALIZED  = -5   # nvram not initialized
PQ_ZK_ERR_YSEC_CONSUMED    = -6   # y_sec already consumed
...
def serialize_merkle_path(depth, leaf_index, siblings, is_right):
    """
    Serialize Merkle path to bytes (matches C serialize_merkle_path)
    Format: depth(4B LE) || leaf_index(4B LE) ||
...
def load_lib(so_path):
    ...
    lib.PQC_eUICC_Init.argtypes = [
        ...
        ctypes.POINTER(ctypes.c_uint8),            # salt (v5.0 new)
        ctypes.POINTER(ctypes.c_uint8), ctypes.c_size_t,  # cred_kyc, len (v5.0 new)
    ]
    ...
    # ---- Phase 4 (v4.0+: R_dynamic replaces H_ctx)----
...
def kat_r_dynamic(lib):
    """
    v4.0+: R_dynamic cross-platform verification

    Important:
      c_layer expected from C test_vectors.
      Steps:
        1. 1. Build and run test_vectors
        2. 2. Find R_dynamic hex in output
        3. 3. Copy to c_layer = bytes.fromhex(...)
      Currently placeholder, will fail as expected.
    """
    ...
    # ─────────────────────────────────────────────────────────────
    print(f"  [TODO] C layer expected = {c_layer_hex}")
    print(f"  [Hint] Compute in C KAT with R_bio=0xAB*32, ctr=42")

    R_dynamic_py2 = hashlib.sha256(R_bio + ctr_le8).digest()
    check("R_dynamic Python determinism", R_dynamic_py == R_dynamic_py2)

    R_dyn2 = hashlib.sha256(R_bio + struct.pack('<Q', ctr_local + 1)).digest()
    check("R_dynamic ctr sensitivity", R_dynamic_py != R_dyn2)

    R_dyn3 = hashlib.sha256(bytes([0xCD]*32) + ctr_le8).digest()
    check("R_dynamic R_bio sensitivity", R_dynamic_py != R_dyn3)

    print(f"  [TODO] Fill c_layer_hex from C layer, enable assert")
...
def kat_prf_mask(lib):
    ...
    lib.PQC_GenerateMask(u8buf(K_sym), u8buf(c_seed),
                         ...
                         u8buf(R_dynamic),
                         ctypes.byref(M2))
    check("GenerateMask determinism",
    ...
    lib.PQC_GenerateMask(u8buf(K_sym), u8buf(c_seed),
                         ...
                         u8buf(R_dynamic),
                         ctypes.byref(M3))
    check("GenerateMask ctr sensitivity",
    ...
    lib.PQC_GenerateMask(u8buf(K_sym), u8buf(c_seed),
                         ...
                         u8buf(R_dynamic2),
                         ctypes.byref(M4))
    check("GenerateMask R_dynamic sensitivity",
...
def kat_gen_challenge(lib):
    print("\n=== Python KAT 4: GenChallenge (v4.0+, no H_ctx) ===")
    ...
    check(f"c_agg hamming weight = {PQ_ZK_CHALLENGE_WEIGHT}",
          weight == PQ_ZK_CHALLENGE_WEIGHT)
    check("c_agg coeffs in {-1,0,1}", bad == 0)

    c2 = poly_t()
    lib.PQC_GenChallenge(ctypes.byref(W), u8buf(c_seed_val),
                         ctypes.byref(c2))
    check("GenChallenge deterministic",
          all(c_agg.coeffs[i] == c2.coeffs[i]
              for i in range(PQ_ZK_N)))


def kat_mac_w_format(lib):
    print("\n=== Python KAT 5: MAC_W format (backend alignment)===")
    ...
    check("EncodePolyVec all-1（first 8 bytes）",
...
def kat_e2e(lib):
    """
    Full protocol end-to-end v5.0

    Phase 3 note:
      Python simulates TEE (computes R_dynamic and AuthToken directly),
      cannot call C TEE_GenerateAuthToken (needs merkle_tree_t).
      hash_M2 uses random placeholder, consistent with AuthToken.
      Correctness: see C test_vectors.c kat_protocol_e2e.
    """
    print("\n=== Python KAT 6: Protocol end-to-end (v5.0) ===")
    ...
    with tempfile.TemporaryDirectory() as nvram_dir:
        ...
        k_tee    = bytes([0x22] * 32)
        R_bio    = bytes([0x33] * 32)
        salt     = os.urandom(32)           # v5.0: device-specific random salt
        cred_kyc = bytes(64)                # simulated, zero-filled
        ...
        weight = sum(1 for i in range(PQ_ZK_N) if c_agg.coeffs[i] != 0)
        check(f"GenChallenge weight={PQ_ZK_CHALLENGE_WEIGHT}",
              weight == PQ_ZK_CHALLENGE_WEIGHT)
        ...
        check("Tampered AuthToken rejected (ERR_MAC_FAIL)",
        ...
        rc_replay = lib.PQC_ComputeZ_and_Mask(
            ...
            u8buf(AuthToken_val),
            ctypes.byref(z_dummy))
        check("Replay rejected (ERR_MAC_FAIL)",
        ...
        check(f"2nd ComputeZ_and_Mask rc={rc2}", rc2 == PQ_ZK_SUCCESS)
        ...
        check("R_dynamic matches", R_dynamic2 == R_dynamic_server)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="PQ-ZK-eSIM v5.0 Python ctypes test")
    ...
    if not os.path.exists(so_path):
        print(f"[ERROR] not found, build first")
        sys.exit(1)

    print("=" * 60)
    print("  PQ-ZK-eSIM Python ctypes test v5.0")
    print(f"  .so: {so_path}")
    print("=" * 60)
