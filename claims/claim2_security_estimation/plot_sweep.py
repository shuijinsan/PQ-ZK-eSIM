#!/usr/bin/env python3
"""Plot the MSIS estimator sweep (Figure 4) from the CSV written by run_sweep.py."""
import csv
import os

from matplotlib import pyplot as plt
from matplotlib.ticker import FuncFormatter

HERE = os.path.dirname(os.path.abspath(__file__))
OUT = os.path.join(HERE, "expected")
HIGHLIGHT = 71400       # chosen extraction bound
TARGET = 128            # engineering threshold on the estimator-reported quantum cost
MLWE_QUANTUM = 162.9    # verified: dual_hybrid, LaaMosPol14

rows = []
with open(os.path.join(OUT, "msis_estimator_sweep.csv")) as f:
    for r in csv.DictReader(f):
        rows.append((int(r["length_bound"]), float(r["classical_bits"]),
                     float(r["quantum_bits"]), int(r["bkz_block_size"])))
xs = [r[0] for r in rows]
cl = [r[1] for r in rows]
qu = [r[2] for r in rows]
bkz = [r[3] for r in rows]

lo = min(min(qu), min(cl), TARGET) - 8
hi = max(max(qu), max(cl)) + 8

fig, (a, b) = plt.subplots(1, 2, figsize=(15, 6.5))
fig.suptitle("Security Estimation (lattice-estimator)", fontweight="bold", fontsize=17)

a.plot(xs, cl, "o-", color="#1f77b4", linewidth=2, markersize=6,
       label="Classical (MATZOV)")
a.plot(xs, qu, "s-", color="#ff7f0e", linewidth=2, markersize=6,
       label="Quantum (LaaMosPol14)")
a.axhline(MLWE_QUANTUM, color="#7f7f7f", linestyle="-.", linewidth=1.5,
          label="MLWE quantum (%.1f)" % MLWE_QUANTUM)
a.axhline(TARGET, color="#d62728", linestyle="--", linewidth=1.5,
          label="128-bit engineering target")
a.axvline(HIGHLIGHT, color="#2ca02c", linestyle="--", linewidth=1.5,
          label="chosen bound = %d" % HIGHLIGHT)
a.set_xlabel("length_bound (norm = infinity)", fontsize=12)
a.set_ylabel("Estimator-reported attack cost (bits)", fontsize=12)
a.set_title("(A) Security Level vs length_bound", fontsize=13)
a.set_ylim(lo, hi)
a.legend(fontsize=9, loc="lower left", framealpha=0.9)
a.grid(alpha=0.3)

b.plot(xs, bkz, "o-", color="#d62728", linewidth=2, markersize=6)
b.axvline(HIGHLIGHT, color="#2ca02c", linestyle="--", linewidth=1.5,
          label="chosen bound = %d" % HIGHLIGHT)
b.set_xlabel("length_bound (norm = infinity)", fontsize=12)
b.set_ylabel("BKZ block size beta", fontsize=12)
b.set_title("(B) Required BKZ-beta vs length_bound", fontsize=13)
b.legend(fontsize=9, loc="upper right", framealpha=0.9)
b.grid(alpha=0.3)

for ax in (a, b):
    ax.xaxis.set_major_formatter(FuncFormatter(lambda v, p: "%dk" % round(v / 1000.0)))

fig.tight_layout()
fig.savefig(os.path.join(OUT, "fig_lattice_estimator.png"), dpi=160)
print("Wrote", os.path.join(OUT, "fig_lattice_estimator.png"))
