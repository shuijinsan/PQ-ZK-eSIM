...
echo "[1/3] Offline eUICC registration ..."
rm -rf "$NVRAM"
"$NATIVE/pqzkesim_setup" --nvram "$NVRAM" --face features.bin --mno-id MNO_A_SIM_001

echo
echo "[2/3] Online authentication (valid proof; expect ACCEPT)..."
"$NATIVE/pqzkesim_app" --auth --nvram "$NVRAM" 2>&1 | tee "$WORK/auth_ok.log" || true
if grep -q "Verify PASS" "$WORK/auth_ok.log"; then
    echo ">>> Result: ACCEPT"
else
    echo ">>> Result: authentication failed (positive case should ACCEPT)"
    exit 1
fi

echo
echo "[3/3] Tamper with registration_data.bin (public key pk_t) to create an invalid proof (expect REJECT)..."
...
echo
echo "==============================================="
echo "  Demo Done: positive ACCEPT / negative REJECT"
echo "==============================================="
